...
inst_acme(){
    ...
    switch_provider
	va=$(~/.acme.sh/acme.sh -v 2>/dev/null)
	if [[ -n $va ]]; then
		version=$(echo "$va" | grep -o 'v[0-9.]*')
		if [ -n "$version"]; then
			echo "Acme.sh 证书一键申请脚本安装成功!版本号为$va."
		else
			echo "Acme.sh 证书一键申请脚本安装成功!版本号获取失败."
		fi
	else
		echo "抱歉, Acme.sh 证书一键申请脚本安装失败"
	fi
}
